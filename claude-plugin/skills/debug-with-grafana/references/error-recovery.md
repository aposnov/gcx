...
**Flags verified against**: `cmd/grafanactl/query/command.go`, `cmd/grafanactl/config/command.go`, `cmd/grafanactl/datasources/`
...
4. Retry the query using the correct UID from the listing output:
   ...
   ```bash
   grafanactl query -d <correct-uid> -e '<expr>' --from now-1h --to now --step 1m -o json
   ```
...
4. Broaden the time range to confirm whether data exists at all:
   ...
   ```bash
   grafanactl query -d <uid> -e '<metric>' --from now-24h --to now --step 5m -o json
   ```
...
5. Simplify the query to remove label filters and verify the base metric returns data:
   ...
   # After (simplified):
   grafanactl query -d <uid> -e 'http_requests_total' --from now-1h --to now --step 1m -o json
   ```
...
1. Reduce the time range to limit the number of data points:
   ...
   ```bash
   grafanactl query -d <uid> -e '<expr>' --from now-30m --to now --step 1m -o json
   ```

2. Increase the step interval to reduce the resolution and query load:

   ```bash
   grafanactl query -d <uid> -e '<expr>' --from now-1h --to now --step 5m -o json
   ```
...
3. Add label filters to reduce cardinality:
   ...
   # After (scoped):
   grafanactl query -d <uid> -e 'rate(http_requests_total{job="api"}[5m])' --from now-1h --to now --step 5m -o json
   ```
...
1. Check that the expression is syntactically complete — all `{`, `[`, and `(` must be closed:
   ...
   # Fixed:
   grafanactl query -d <uid> -e 'rate(http_requests_total{job="api"}[5m])' --from now-1h --to now --step 1m -o json
   ```
...
3. Verify label values use double quotes, not single quotes or no quotes:
   ...
   # Wrong: {job=api}
   # Correct:
   grafanactl query -d <uid> -e '{job="api"} |= "error"' --from now-1h --to now -o json
   ```
...
4. For rate and increase functions, always specify the range window:
   ...
   # Correct:
   grafanactl query -d <uid> -e 'rate(http_requests_total{code="500"}[5m])' --from now-1h --to now --step 1m -o json
   ```
