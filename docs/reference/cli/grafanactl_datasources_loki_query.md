...
```
grafanactl datasources loki query [DATASOURCE_UID] EXPR [flags]
```
...
```
  ...
  grafanactl datasources loki query loki-001 '{job="varlogs"}' --from now-1h --to now

  # With custom limit
  grafanactl datasources loki query loki-001 '{job="varlogs"}' --from now-1h --to now --limit 500

  # No limit (return all matching log lines)
  grafanactl datasources loki query loki-001 '{job="varlogs"}' --limit 0

  # Output as JSON
  grafanactl datasources loki query loki-001 '{job="varlogs"}' -o json
```
