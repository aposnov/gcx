## grafanactl datasources generic query
...
```
grafanactl datasources generic query DATASOURCE_UID EXPR [flags]
```
...
```
  ...
  grafanactl datasources generic query ds-001 'up{job="grafana"}' --from now-1h --to now

  # Loki via generic (with limit)
  grafanactl datasources generic query loki-001 '{job="varlogs"}' --from now-1h --to now --limit 200

  # Pyroscope via generic
  grafanactl datasources generic query pyro-001 '{service_name="frontend"}' \
