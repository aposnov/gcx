## grafanactl datasources pyroscope query
...
```
grafanactl datasources pyroscope query [DATASOURCE_UID] EXPR [flags]
```
...
```
  ...
  grafanactl datasources pyroscope query pyro-001 '{service_name="frontend"}' \
    --profile-type process_cpu:cpu:nanoseconds:cpu:nanoseconds \
    --from now-1h --to now

  # Using configured default datasource
  grafanactl datasources pyroscope query '{service_name="frontend"}' \
    --profile-type process_cpu:cpu:nanoseconds:cpu:nanoseconds \
    --window 1h
